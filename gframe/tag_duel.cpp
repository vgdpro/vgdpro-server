...
void TagDuel::TPResult(DuelPlayer* dp, unsigned char tp) {
	...
	// preload_script(pduel, "./script/VgDefinition.lua", 0);
	// preload_script(pduel, "./script/VgFuncLib.lua", 0);
	// preload_script(pduel, "./script/VgD.lua", 0);
...
if(!dp || dp == players[pid])
	...
	if(!dp || dp == players[pid + 1])
		NetServer::SendBufferToPlayer(players[pid + 1], STOC_GAME_MSG, query_buffer.data(), len + 4);
	if(!dp)
		NetServer::ReSendToPlayer(replay_recorder);
#else
